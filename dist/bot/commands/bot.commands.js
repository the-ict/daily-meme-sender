import { etc_start, first_start, help_message, } from "../../constants/messages.js";
...
const addMeme = (bot) => {
    bot.command("submit", async (ctx) => {
        await ctx.reply("📸 Rasm yuboring va caption da mood ni ko'rsating.\n\n" +
    ...
    bot.on("photo", async (ctx) => {
        try {
            ...
            await meme.save();
            await ctx.reply(`✅ Meme saqlandi!\n\n` +
                `🆔 \`${meme._id}\`\n` +
                `📸 ${meme.caption ? meme.caption.replace(/[<>]/g, '') : "Mavjud emas!"}\n` +
                `😊 Mood: ${mood}`, { parse_mode: "HTML" });
        }
        catch (error) {
            console.error(error);
...
const mood = (bot) => {
    bot.command("mood", async (ctx) => {
        ...
        await ctx.reply(`😊 Qaysi kayfiyatda memes ko'rmoqchisiz?\n\n📊 *Mood statistikasi:*\n${statsText}`, {
            parse_mode: "HTML",
            reply_markup: {
                inline_keyboard: [
                    [
...
const mymemes = (bot) => {
    bot.command("mymemes", async (ctx) => {
        ...
        userMemes.forEach((meme, index) => {
            message += `${index + 1}. 👍 ${meme.up.length} | 👎 ${meme.down.length} | 👁 ${meme.views}\n`;
            message += `${meme.caption ? meme.caption.replace(/[<>]/g, '') : "Meme"}\n\n`;
        });
        await ctx.reply(message, {
            parse_mode: "HTML",
...
const streak = (bot) => {
    bot.command("streak", async (ctx) => {
        ...
        await ctx.reply(`🔥 *Sizning streak'ingiz:* ${diffDays}\n\n` +
...
const about = (bot) => {
    bot.command("about", async (ctx) => {
        await ctx.reply(`🤖 <b>MemeMaster Bot</b>\n\n` +
            `Versiya: 1.0.0\n` +
            `Til: TypeScript\n` +
            `Framework: Telegraf\n\n` +
...
const report = (bot) => {
    bot.command("report", async (ctx) => {
        await ctx.reply(`🛠️ <b>Xatolik yuborish</b>\n\n` +
...
export { help, start, getTgId, addMeme, daily, battle, mood, language, random, top, mymemes, streak, about, report, };
